...
Статус: **E6A в работе** — готовы оболочка курса, публикация и серверный поиск; далее публичный каталог и сохранения. [Состояние и оставшиеся задачи](docs/05-status.md).
...
Дальше в четырёх терминалах:

```bash
cd apps/api && uv run uvicorn app.main:app --reload   # API      → http://localhost:8000
cd apps/api && uv run arq app.worker.WorkerSettings   # импорт, экспорт аккаунта и индекс курсов
...
Сквозная проверка поискового индекса требует локальных PostgreSQL, Redis, MailHog и Meilisearch:

```bash
cd apps/api && RUN_SEARCH_INTEGRATION=1 uv run pytest tests/test_search_live.py
```

Тест использует тестовую БД и отдельные очередь и индекс; временный индекс удаляется после
проверки. Worker синхронизирует каталог при старте и раз в минуту. Доступ к результатам
проверяется по PostgreSQL, поэтому снятый с публикации курс скрывается до обновления индекса.
