...
Полный утверждённый набор логотипов находится в `assets/brand/`; описание файлов —
`assets/brand/README.md`. Мастер-знак, компоновки, favicon и PWA-иконки нельзя перерисовывать
или менять вручную. Производные файлы для `packages/ui`, `apps/web`, `apps/app` и `apps/mobile`
пересобираются командой `scripts/generate-brand-assets.sh`. Правила цветов, размеров и применения
зафиксированы в разделе «Бренд» файла `design.md`.
...
редакционный блог опубликован, подборки реализованы локально и ждут наполнения/выпуска.
Далее производительность и постмодерация; редакционный процесс — `docs/11-collections.md`.
Главная, блог и предыдущие срезы опубликованы на дев-сервере; Git push сам по себе не является деплоем.
