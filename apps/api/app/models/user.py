...
class UserSettings(UUIDPrimaryKeyMixin, Base):
    ...
    daily_goal_cards: Mapped[int] = mapped_column(default=20)
    fsrs_desired_retention: Mapped[float] = mapped_column(default=0.90)
    fsrs_max_interval_days: Mapped[int] = mapped_column(default=365)
    new_cards_per_day: Mapped[int] = mapped_column(default=20)
    reviews_per_day: Mapped[int] = mapped_column(default=200)
    tts_voice_preference: Mapped[str | None] = mapped_column(String(32))
    notification_channels: Mapped[dict[str, Any]] = mapped_column(JSONB, default=dict)
