...
Turn the user's sources into private learning materials. The external agent generates content;
Remora stores it. Do not promise that Remora itself runs an AI model.
...
## Saving is not publication

«Добавь на сайт», «сохрани» and «создай курс» authorize private creation, not public access.
Only a course can be published. A standalone set has no publish tool; offer a course wrapper
when public access is requested. If the user explicitly requested publication of a defined
material, that is sufficient authorization; do not demand a redundant confirmation.
First create/read back the private course and check the publication requirements in formats.md,
then call publish_course with a separate request_key and publication.tags. Verify is_published
afterwards. A failed publication does not undo private creation: report the saved course ID and
