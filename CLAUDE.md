...
Единственное исключение — фирменный знак: у логотипа нет светлой и тёмной версии,
его цвета зафиксированы в `design.md`, а геометрия — в `packages/ui/src/assets/remora-mark.svg`.
Перерисовывать знак или переводить его на токены нельзя.
...
E2, E3, E4 и E6B завершены. E5 завершён, кроме аудиовложений Anki: они ждут появления
аудио в модели карточки.

Идёт E6A. Публикация курсов, каталог и поиск, лайки и сохранения, профили авторов, SEO,
главная и блог уже работают на дев-сервере. В самом E6A остаются постмодерация и жалобы,
инкрементальная синхронизация Meilisearch, независимое копирование набора из публичного
курса, наполнение редакционных подборок и производительность публичного слоя.
Заявленный следующий приоритет после хвоста E6A — боты: живая проверка привязки и отзыва,
алерты доставки и мини-приложение.

Этапы E7–E12 не начинались. Текущий прогресс — `TODO.md`, отчёт — `docs/05-status.md`,
полный план — `docs/03-roadmap.md`. При расхождении верить `TODO.md` и коду.
